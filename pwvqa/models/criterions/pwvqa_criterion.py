...
class PWVQACriterion(nn.Module):

    def __init__(self, question_loss_weight=1.0, vision_loss_weight=1.0, is_va=True):
        super().__init__()
        self.is_va = is_va

        Logger()(f'PWVQACriterion, with question_loss_weight = ({question_loss_weight})')
        if self.is_va:
            Logger()(f'PWVQACriterion, with vision_loss_weight = ({vision_loss_weight})')
